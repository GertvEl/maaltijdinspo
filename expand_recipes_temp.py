...
with path.open('r', encoding='utf-8') as f:
    existing_recepten = json.load(f)

# Add enough recipes to reach 100 total while keeping the categories balanced.
existing_count = len(existing_recepten)
needed = 100 - existing_count
counts = {'kip': 17, 'rund': 17, 'zalm': 17, 'kabeljauw': 17, 'pasta': 16, 'vegetarisch': 16}
# Keep the initial 16 recipes and add 84 more to reach 100 total.
# The counts below sum to 84, matching the remainder after the existing dataset.
added_counts = {'kip': 13, 'rund': 14, 'zalm': 15, 'kabeljauw': 15, 'pasta': 14, 'vegetarisch': 13}
...
new_recipes = []
seen_ids = {r.get('id') for r in existing_recepten}
seen_names = {r.get('naam') for r in existing_recepten}
for category, total in added_counts.items():
...
updated = existing_recepten + new_recipes
...
print('written_recipes', len(updated))
print('category_counts', {c: sum(1 for r in updated if r.get('categorie') == c) for c in ['kip', 'rund', 'zalm', 'kabeljauw', 'pasta', 'vegetarisch']})
